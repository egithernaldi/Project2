...
<div class="row">
    <div class="col mb-4">
        ...
        @if ($skck->pengajuan->status == 0 || $skck->pengajuan->status == 3)
            <form action="{{ route('admin-skck-approve', $skck->pengajuan_id) }}" method="post" class="d-inline-block">
                @csrf
            ...
            <form action="{{ route('admin-skck-reject', $skck->pengajuan_id) }}" method="post" class="d-inline-block">
                @csrf
            ...
            <form action="{{ route('admin-skck-revisi', $skck->pengajuan_id) }}" method="post" class="d-inline-block">
                @csrf
...
<div class="row">
    ...
    <div class="col-lg-6">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" value="{{ $skck->pengajuan->created_at }}" disabled>
                </div>

                <div class="form-group">
                    <label for="keperluan">Keperluan <span class="text-danger">*</span></label>
                    <textarea type="text" class="form-control" id="keperluan" name="tujuan" disabled>{{ $skck->tujuan }}</textarea>
                </div>
                <div class="form-group">
                    <label class="d-block">Scan KTP</label>
                    @if ($skck->pengajuan->scan_ktp)
                        <a href="{{ url('storage/'.$skck->pengajuan->scan_ktp) }}" class="btn btn-sm btn-outline-secondary" target="_blank" rel="noopener noreferer">
                            Lihat KTP
                        </a>
                    @else
                        <i>Tidak ada KTP</i>
                ...
                <div class="form-group">
                    ...
                    @if ($skck->pengajuan->scan_kk)
                        <a href="{{ url('storage/'.$skck->pengajuan->scan_kk) }}" class="btn btn-sm btn-outline-secondary" target="_blank" rel="noopener noreferer">
                            Lihat KK
                        </a>
                    @else
                        <i>Tidak ada KK</i>
                ...
                <div class="form-group">
                    ...
                    @if ($skck->pengajuan->scan_akta)
                        <a href="{{ url('storage/'.$skck->pengajuan->scan_akta) }}" class="btn btn-sm btn-outline-secondary" target="_blank" rel="noopener noreferer">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin-skck-comment', $skck->pengajuan_id) }}" method="POST">
                    @csrf
                    <div class="form-group">
